...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "lockedcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "lockedcode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "lockedcode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "lockedcode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["lockedcode.json", "lockedcode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "lockedcode.json"),
        path.join(tmp.path, "lockedcode.jsonc"),
        path.join(project, "lockedcode.json"),
        path.join(project, "lockedcode.jsonc"),
      ])
    })
  })
